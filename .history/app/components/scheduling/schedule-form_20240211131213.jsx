...
export default function schedulehtmlForm({children,show,setShow}) {
    ...
    if(e.target.checked){
      setSchedule((prev)=>{
        if(prev.platforms)
          ...
          return {...prev,platforms:[e.target.value]};
      });
    }
  }
  ...
  const submitHandler=()=>{
    if(!schedule.date||!schedule.time||!schedule.repeat||!schedule.platforms){
    ...
    <div id="scheduling-htmlForm" className={`${show?'block':'hidden'} z-40 fixed top-0 left-0 h-screen w-screen`} style={{backgroundColor: 'rgba(0,0,0,.5)'}}>
      <div className="bg-base-100 fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 rounded-xl p-4 max-h-screen overflow-y-scroll opacity-100">
        ...
        <div className="flex justify-between items-center m-4">
          <input onChange={checkBoxHandler} value="facebook" type="checkbox" className="form-checkbox checkbox checkbox-xl checkbox-primary" />
          ...
          <input onChange={checkBoxHandler} value="linkedin" type="checkbox" className="form-checkbox checkbox checkbox-xl checkbox-primary" />
        ...
        <div className="flex flex-col m-4">
          ...
          <input type="date" id="date" onChange={inputHandler} name="date" className="form-input input input-primary input-bordered" />
          <label htmlFor="time">Time:</label>
          <input type="time" id="time" onChange={inputHandler} name="time" className="form-input input input-primary input-bordered" />
          <label htmlFor="repeat">Repeat:</label>
          <select onChange={inputHandler} name="repeat" id="repeat" className="form-select select select-bordered select-primary w-full">
        ...
        <div className="flex justify-center items-center m-4">
          <button onClick={submitHandler} className="btn btn-primary">Schedule</button>
        </div>
      </div>
    </div>
  )
