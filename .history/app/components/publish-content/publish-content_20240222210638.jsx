            ...
            options.map((option,index)=>{
              return (
                <div key={index} className="card card-body p-4 flex flex-col items-center gap-4">
                  ...
                  <div>{option.name}</div>
                  <input onChange={(e)=>{setPlatforms(prevPlatforms=>[...prevPlatforms,e.target.value])}} type="checkbox" value={option.value} className="form-checkbox checkbox checkbox-xl checkbox-primary" />
                </div>
              )
            })
          }
