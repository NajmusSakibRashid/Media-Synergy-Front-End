    ...
    <div className="card shadow-xl w-full bg-base-200 p-4 gap-4">
      ...
      <div className="flex gap-4">
        ...
        <div className="basis-1/2 card-body flex flex-col w-full gap-2 jsutify-around">
          ...
          {content?.media?.map(elem => <img src={elem} className="w-full" />)}
          <input onChange={(e) => { console.log(e.target.files);setContent({...content,media:e.target.files.map(elem=>URL.createObjectURL(elem))})}} name='media' type="file" className="file-input file-input-bordered w-full " />
        </div>
      </div>
